...
class AmneziaTrafficCollectorTests(unittest.TestCase):
    ...
    def test_get_path_mtu_probe_ignores_cache_for_different_target(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir, ExitStack() as stack:
            self._patch_repo_paths(stack, Path(temp_dir))
            stack.enter_context(patch.object(collector, "MTU_PROBE_TARGET", "9.9.9.9"))
            stack.enter_context(patch.object(collector, "MTU_PROBE_CACHE_SECONDS", 3600))
            stack.enter_context(patch.object(collector, "now_local", return_value=self.current_time))
            collector.save_json(
                collector.TRANSPORT_CACHE_FILE,
        ...
        self.assertEqual(probe["target"], "9.9.9.9")
        self.assertEqual(probe["estimated_path_mtu"], 1500)
    ...
    def test_collect_writes_summary_and_dashboard_outputs(self) -> None:
        ...
        self.assertIn("Autostop VPN Shell", html)
        self.assertNotIn("F5", html)
        self.assertIn("traffic-banner", html)
        self.assertIn("Загрузка канала", html)
        self.assertIn('id="peer-search"', html)
        self.assertIn('id="status-filter"', html)
        self.assertIn('id="location-filter"', html)
        self.assertIn("detail-panel", html)
        self.assertIn("Лидер трафика", html)
        self.assertIn("status-dot active", html)
        self.assertIn("share-cell", html)
    ...
    def test_doctor_reports_ready_environment(self) -> None:
        ...
        self.assertIn("[OK] shell launcher", output)
        self.assertIn("[OK] shell app", output)
